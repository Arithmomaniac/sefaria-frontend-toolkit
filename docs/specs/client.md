...
- the reviewed deterministic overlay, including semantic-preserving reusable schema components
- generated TypeScript request, response, component, and operation types
- generated Zod schemas and TypeScript runtime validators
- 60 SDK functions generated by `@hey-api/openapi-ts` and exposed through 11 tag namespaces, with `@hey-api/client-fetch` as an external runtime dependency
- a configurable generated fetch client with an injectable `fetch`
- a bounded per-client response cache for validated public-data GET responses

The package does not own component view models, normalized domain models, rendering, retries, request coalescing, persistent or cross-client caches, stale fallback, or component-specific methods.
...
Generation and contract checks cover all 60 operations in the pinned document. Each operation appears exactly once in the namespace corresponding to its OpenAPI tag.

| OpenAPI tag | Public namespace | Operation count |
| ----------- | ---------------- | --------------: |
| Text        | `text`           |               9 |
| Index       | `index`          |              10 |
| Related     | `related`        |               5 |
...
The scope is the pinned OpenAPI document, not every Django route. Flat endpoint exports are intentionally absent during the package's alpha phase.
...
flowchart LR
    ...
    ASSERT --> OVERLAY["Reviewed corrections"]
    OVERLAY --> CORRECTED["Temporary corrected API document"]
    CORRECTED --> TYPES["Generated request, response, and component types"]
    CORRECTED --> SCHEMAS["Generated Zod schemas"]
...
- a co-located `x-sefaria-guards` precondition with the expected old value or explicit expected absence
- the corrected value, or an explicit removal
- a stable correction identifier that maps to source provenance in [evidence](../evidence.md)
- a dated fixture in the fixture manifest when runtime data affects the shape
...
If an overlay guard fails, generation stops before overlay application. The error reports the correction identifier, JSONPath target, expected state, and actual state.
...
Generation applies the overlay and retains the complete pinned 60-operation document in a temporary corrected OpenAPI artifact. That temporary document is the only input for TypeScript and Zod generation.

The corrected document must be byte-stable for the same pinned input, checksum, overlay, and generator versions. It is not committed or exported from the package.
...
## Public runtime schemas and validators

The package publishes Zod schemas and TypeScript runtime validators generated from the temporary corrected document.
...
The public client validates every JSON response against the schema for its operation and status. This validation applies to documented success and error responses. A declared `image/png` response is checked against its documented media type and returned as a `Blob` without JSON parsing.
...
The public client exports generated namespace objects named `text`, `index`, `related`, `calendars`, `lexicon`, `topic`, `term`, `sheets`, `collections`, `misc`, and `ref`. It does not export endpoint functions directly, add handwritten endpoint request functions, or add a generalized facade.
...
Each namespaced SDK function accepts the generated client in its options. The package also exports generated operation types, Zod schemas, and response validators.
...
The cache covers completed, validated HTTP 200 responses from eligible public-data GET operations, including PNG images. It explicitly bypasses `/api/async/{task_id}`, `/api/sheets/modified/{sheet_id}/{timestamp}`, `/api/texts/random`, and `/api/texts/random-by-topic`; both POST operations are also uncached.

The cache key comes from the finalized Fetch request and distinguishes the absolute URL, repeated query values and their order, request headers, and Fetch representation options. It does not normalize references, infer equivalence between query forms, or fulfill one operation from another operation's response.

The cache stores valid empty and partial success payloads. It does not store documented HTTP errors, HTTP 200 error objects, unresolved `{ "is_ref": false }` responses, malformed or mismatched data, network failures, aborts, failed body streams, authenticated requests, responses forbidden by HTTP cache metadata, or oversized bodies. A cache miss or ineligible request uses the ordinary generated-client path.

TTL starts when a validated response is admitted and does not extend on access. Least-recently-used eviction enforces both entry and retained-body-byte limits. Retained-body bytes are measured from the decoded response body and do not claim to bound total JavaScript heap usage.

Cache hits return independent response clones and pass through the same status-aware generated validation as network responses. Clearing a client cache is synchronous and prevents requests begun before the clear from repopulating it. The cache does not persist, share state between clients, return stale values, retry, prefetch, reconstruct ranges, or coalesce concurrent misses.

Requests carrying authorization or cookie headers, requests whose credentials can carry same-origin cookies, and requests with explicit non-default Fetch cache directives bypass the application cache. Hosts using injected fetch implementations with hidden authentication must disable caching or isolate client lifetimes appropriately.
...
- all 60 operation types and 11 namespace memberships
...
- default, disabled, and configured per-client response caching
- exact key separation for query order, headers, origins, and Fetch options
- TTL, LRU entry eviction, retained-body-byte eviction, and explicit clearing
- cached response metadata, independent body consumption, and current-signal abort behavior
- cache bypass for errors, negative results, polling, random-result operations, submissions, authentication, and restrictive HTTP metadata
- PNG media-type validation, byte preservation, and cache replay without JSON parsing
...
Each corrected schema must pass fixtures derived from the original handler, response builder, and upstream endpoint tests or a recorded local-data recipe where stored variants drive the contract.
...
- temporary full-document generation covers all 60 operations and 11 namespaces
- generated output is current
- public validators report structured paths
- every JSON response passes runtime validation or rejects with a structured contract mismatch, and declared PNG responses preserve binary semantics
- the generated client preserves success, HTTP error, network, and abort semantics
- the bounded per-client response cache preserves validation and Fetch semantics
- no generalized model facade, retry, coalescing, stale fallback, persistence, cross-client sharing, or component method exists
