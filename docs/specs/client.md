...
- the reviewed deterministic Core overlay, including semantic-preserving reusable schema components
...
- eight named SDK functions generated by `@hey-api/openapi-ts` with `@hey-api/client-fetch` as an external runtime dependency
...
| `POST` | `/api/find-refs` | Enqueue citation detection for supplied title and article text |
| `GET` | `/api/async/{task_id}` | Poll the state and result of the known citation-detection task |
...
flowchart LR
    ...
    OVERLAY --> CORRECTED["Temporary corrected Core document"]
...
Generation applies the overlay and extracts the eight Core operations into a temporary corrected OpenAPI document. That temporary document is the only input for TypeScript and Zod generation.
...
The cache covers completed, validated HTTP 200 responses from the six public-data GET operations: v3 texts, versions, reference information, index metadata, shape, and links. It does not cover citation submission or asynchronous task polling.
...
- cache bypass for errors, negative results, polling, submissions, authentication, and restrictive HTTP metadata
...
- temporary Core extraction covers all Core operations
