...
The production editor build was qualified at `/` and `/sefaria-frontend-toolkit/`, directly and when embedded from an ordinary same-origin documentation page. Its real Rollup graph contained shared generated chunks mapped to local data modules. In each engine, importing the pure source-card subpath did not register the element, the client root and validation subpath exposed the same `getResponseContract` function identity, and importing the web-components root registered the same `SefariaSourceCard` constructor exactly once. The maintained Micah 6:8 project rendered with no external requests or page errors.

The same acceptance sent fetch, beacon, WebSocket, image, nested-frame, and navigation attempts to a separately counted loopback probe server. The probe server received zero requests. Focused tests also proved draft isolation until Run, Reset restoration, one active preview, keyboard tab selection, narrow 200%-equivalent layout, exact source/message/diagnostic/preview bounds, and visible runtime/import/CSP diagnostics. This is browser confinement evidence, not CPU, heap, process isolation, every-Safari compatibility, or guaranteed recovery from a tight loop.
