...
Presentation properties operate on the element's current private prepared content:
...
The equivalent authored markup is `<button slot="toolbar-actions">Bookmark selected text</button>`. The generic supplied-data Reader project demonstrates editable Reader HTML, CSS, and JavaScript, but the maintained standalone Reader linked below is the proof for this host-owned bookmark action.
...
The slot is available after Reader content commits; the full initial loading state intentionally has no toolbar. Read the action target from the Reader's current `selectedRef` when the action runs rather than capturing an earlier reference. `toolbar`, `history`, `source-pane`, and `connections-pane` are the complete Reader part set. There are no forwarded child parts. Prefer shared tokens for theme-wide changes; if a `::part` rule changes display, overflow, or layout, the host owns the resulting responsive and accessibility behavior.

For a headless path, import only the transport and text layers you need:
...
- Standalone Reader customization: [`examples/reader/src/controlled-app.ts`](https://github.com/Arithmomaniac/sefaria-frontend-toolkit/blob/main/examples/reader/src/controlled-app.ts)
